...
class Marina {
	...
	// a corner is "covered" when another corner exists to its (left, right, top, bottom)
	// along a continuous edge.  The point is to collect information that can be used to
	// eliminate a corner from the corners list because it does not protrude from the
...
// add dock corners to array so that they can be used to determine when the boat has
// collided with the dock.  Eliminate corners that are "buried" (in other words, given
// the structure of the dock, these are corners that the boat would not be able to hit).
